...
class ReviewNetworkMultilayerModel(object):
    ...
    def create_model(self, input_seqs, image_model_output, initializer,
        ...
        with tf.variable_scope('lstm_layer_2') as scope_2:
            lstm_review = tf.contrib.rnn.BasicLSTMCell(num_units=FLAGS.num_lstm_units,state_is_tuple=True)
            if mode=='train':
                lstm_review = tf.contrib.rnn.DropoutWrapper(
                    ...
                    output_keep_prob=FLAGS.lstm_dropout_keep_prob)
            if FLAGS.use_attention_wrapper:
                if self.use_concate_image:
                    attention_mechanism = getattr(tf.contrib.seq2seq, FLAGS.attention_mechanism)(
                        num_units=FLAGS.num_attention_depth,
                        memory=tf.concat(axis=-1, values=[lstm_outputs,middle_layer])
                    )
                else:
                    attention_mechanism = getattr(tf.contrib.seq2seq, FLAGS.attention_mechanism)(
                        num_units=FLAGS.num_attention_depth,
            ...
            if mode=='train':
                batch_size = get_shape(image_embeddings)[0]
                zero_state = lstm_review.zero_state(batch_size=batch_size, dtype=tf.float32)
                _, initial_state = lstm_review(image_embeddings,zero_state)
                sequence_length = tf.reduce_sum(input_mask,1)
                lstm_outputs_final, _ = tf.nn.dynamic_rnn(cell=lstm_review,
                                                    inputs=seq_embeddings,
                                                    sequence_length=sequence_length,
                                                    initial_state=initial_state,
                                                    dtype=tf.float32,
                                                    scope=scope_2)
                lstm_outputs_final = tf.reshape(lstm_outputs_final,[-1,lstm_review.output_size])
        with tf.variable_scope("logits") as logits_scope:
            logits = tf.contrib.layers.fully_connected(
                inputs=lstm_outputs_final,
