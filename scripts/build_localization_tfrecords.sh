...
TEST2_IMAGE_DIR="${DIR}/../data/ai_challenger_caption_test_b_20171120/caption_test_b_images_20171120"
TEST2_LOCALIZATIONS_FILE="${DIR}/../data/bottom_up_attention/aichallenger_test2.tsv.small"
...
if [ ! -f $OUTPUT_DIR/test2-00000-of-00008.tfrecord ]; then
  # run the script
  CUDA_VISIBLE_DEVICES="1" python ${DIR}/build_localization_tfrecords.py \
    --output_dir=$OUTPUT_DIR \
    --word_counts_input_file=$VOCAB_FILE \
    ...
    --test1_image_dir=$TEST1_IMAGE_DIR \
    --test1_localizations_file=$TEST1_LOCALIZATIONS_FILE \
    --test2_image_dir=$TEST2_IMAGE_DIR \
    --test2_localizations_file=$TEST2_LOCALIZATIONS_FILE \
    --build_flip_caption=True \
    --task=test2 \
    --min_word_count=$MIN_WORD_COUNT
fi
