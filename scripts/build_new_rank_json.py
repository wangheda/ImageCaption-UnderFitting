...
def _to_json_example(image_id, image_filename, image_captions, caption_scores, decoder, vocab):
    with tf.gfile.FastGFile(image_filename, "r") as f:
        encoded_image = f.read()
    try:
        decoder.decode_jpeg(encoded_image)
    ...
    example = {
        ...
        "image/seqlens": seqlens,
        "image/scores": caption_scores,
        "image/captions": caption_ids,
    }
    return example
...
def _write_to_file(filename, strings):
    ...
    json.dump(strings, output, indent=4, ensure_ascii=False)
    output.close()
...
def _process_dataset(image_metadata, vocab):
    ...
    examples = []
    counter = 0
    print("find %d images" %(len(image_metadata)))
    for image_id, filename, captions, caption_scores in image_metadata:
        sequence_example = _to_json_example(image_id, filename, captions, caption_scores, decoder, vocab)
        if sequence_example is not None:
            examples.append(sequence_example)

        if len(examples) >= images_per_file:
            _write_to_file(os.path.join(FLAGS.output_dir, "%s-%d.json" % (FLAGS.output_prefix, counter)), examples)
            examples = []
            counter += 1
