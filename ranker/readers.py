...
class ImageCaptionReader(BaseReader):
  ...
  def prepare_reader(self, filename_queue, batch_size=16):
    ...
    feature_map = {
        ...
        "image/pos_seqlens": tf.FixedLenFeature([self.lines_per_image], tf.int64),
        "image/neg_seqlens": tf.FixedLenFeature([self.lines_per_image], tf.int64),
        "image/pos_captions": tf.FixedLenFeature([num_words], tf.int64),
        "image/neg_captions": tf.FixedLenFeature([num_words], tf.int64),
      }
...
class ImageCaptionTestReader(BaseReader):
  ...
  def prepare_reader(self, filename_queue, batch_size=16):
    ...
    seqlens = tf.reshape(features["image/seqlens"], 
                              shape=[self.lines_per_image])
    print(" captions", captions, seqlens)

    return image_ids, images, captions, seqlens
...
def get_input_data_tensors(data_pattern=None,
  ...
  if is_training:
    ...
    reader = ImageCaptionTestReader(lines_per_image=FLAGS.lines_per_image,
                                max_len=FLAGS.max_len)
